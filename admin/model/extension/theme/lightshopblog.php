...
class ModelExtensionThemeLightshopBlog extends Model {
	...
	public function editBlog($blog_id, $data) { 
		$this->db->query("UPDATE " . DB_PREFIX . "lightshop_blog SET sort_order = 0, bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', date_added = '" . $data['date_added'] . "' WHERE blog_id = '" . (int)$blog_id . "'");
