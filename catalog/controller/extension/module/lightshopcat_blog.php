...
class ControllerExtensionModuleLightshopcatblog extends Controller {
	public function index() {
		...
		$blogcategories = $this->model_extension_module_lightshopcatblog->getBlogCategories(0);

		foreach ($blogcategories as $blogcategory) {
			$children_data = array();

			if ($blogcategory['category_id'] == $data['category_id']) {
				$children = $this->model_extension_module_lightshopcatblog->getBlogCategories($blogcategory['category_id']);
			...
			$data['blogcategories'][] = array(
				...
				'count'        => $this->model_extension_module_lightshopblog->getTotalBlogs($filter_data),
...
//lightshop start
			...
			if ($category_id) {
				...
				$blogCat = array();
				$catinfo = $this->model_extension_module_lightshopcatblog->getBlogCategory($category_id);
				if($catinfo){
					$blogCat = array(
						'name'		=>	$catinfo['name'],
